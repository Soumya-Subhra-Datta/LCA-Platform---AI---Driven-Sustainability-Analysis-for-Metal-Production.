...
The React frontend (authentication + future dashboard) lives in `frontend/`. It is a Vite + React SPA served by a small Express server that proxies `/api` to the FastAPI backend.

```bash
cd frontend
npm install

# Development (Vite dev server on :3000, proxies /api to :8000)
npm run dev

# Production (build + Express on :3000)
npm run build
npm start
```

Notes:
- Copy `frontend/.env.example` to `frontend/.env` only if you need a non-same-origin API base URL (`VITE_API_BASE_URL`).
- The production Express server uses `API_PROXY_TARGET` (default `http://localhost:8000`) to forward `/api` and `/health` to the backend.
- Windows only: if `npm install`/`npm run` fails with `spawn C:\Program Files\nodejs\ ENOENT` (npm 10.9.x bug), run:
  `npm config set script-shell C:\Windows\System32\cmd.exe`

### Authentication routes
- `GET /login` - Login page
- `GET /` - Redirects to `/login` (or a minimal signed-in placeholder after login)
- Signup opens as a modal over the login page
...
The repository includes a `render.yaml` blueprint for a two-service deployment:

1. **Backend** (`lca-platform-backend`) - FastAPI + ML models + LCA engine
2. **Frontend** (`lca-platform-frontend`) - React SPA on Express, proxying `/api` to the backend

Steps:
1. Push this repository to GitHub.
2. On [Render](https://render.com), choose **New > Blueprint** and select the repository.
3. When prompted, set `DATABASE_URL` to your database (e.g. TiDB Serverless):

   ```
   mysql+pymysql://USER:PASSWORD@GATEWAY_HOST:4000/lca_platform?ssl_verify_cert=true&ssl_verify_identity=true
   ```

   `APP_SECRET_KEY` is auto-generated by the blueprint.

Optionally, the frontend build can also be hosted as static files on **Vercel** (serve `frontend/dist` and rewrite `/api/*` to the backend URL).
